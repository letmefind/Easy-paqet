...
اگه حروف فارسی در terminal برعکس نمایش داده می‌شن، این مشکل از terminal emulator شماست، نه از اسکریپت.

## راه حل‌های سریع:

### 1️⃣ استفاده از Terminal بهتر

برخی terminal emulator ها بهتر از RTL (Right-to-Left) پشتیبانی می‌کنن:
...
اگه از SSH استفاده می‌کنی، می‌تونی از tmux یا screen استفاده کنی:
...
### 5️⃣ تنظیمات PuTTY (اگه استفاده می‌کنی)
...
اگه هیچکدوم کار نکرد، می‌تونی از اسکریپت انگلیسی استفاده کنی:

```bash
./setup_en.sh
```

یا می‌تونی از فایل‌های کانفیگ آماده استفاده کنی و دستی ویرایش کنی:
...
اگه متن برعکس نمایش داده شد، terminal شما مشکل داره.
...
**نکته:** اسکریپت‌های setup خودشون سعی می‌کنن locale رو تنظیم کنن، اما اگه terminal شما از RTL پشتیبانی نکنه، باز هم مشکل خواهی داشت.
