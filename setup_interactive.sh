...
echo ""
read -p "نام اینترفیس شبکه را وارد کن (مثلا eth0, ens3): " INTERFACE
...
if [ -z "$LOCAL_IP" ]; then
    ...
    read -p "آیا این آی‌پی درست است؟ [Y/n]: " confirm_ip
...
if [ -n "$GATEWAY_IP" ]; then
    ...
    if [ -n "$ROUTER_MAC" ]; then
        ...
        read -p "آیا این MAC آدرس درست است؟ [Y/n]: " confirm_mac
        if [[ "$confirm_mac" == "n" || "$confirm_mac" == "N" ]]; then
...
if [ "$ROLE" == "server" ]; then
    ...
    read -p "آیا می‌خوای قوانین iptables اعمال بشن؟ [Y/n]: " apply_iptables
    
    if [[ "$apply_iptables" != "n" && "$apply_iptables" != "N" ]]; then
...
if command -v paqet &> /dev/null; then
    ...
    if [ -f "./paqet" ]; then
        ...
        read -p "آیا می‌خوای الان دانلود کنی؟ [Y/n]: " download_paqet
        
        if [[ "$download_paqet" != "n" && "$download_paqet" != "N" ]]; then
